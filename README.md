...
**Project Overview:**
In this project, I designed and implemented a Continuous Integration and Continuous Deployment (CI/CD) pipeline for a Spring Boot application using various tools, including Jenkins, Git, Maven, SonarQube, Nexus, Docker and Tomcat. The primary goal was to automate the software development lifecycle, ensuring faster releases and better code quality.
...
Artifact Management: Once the build is successful and the code quality is validated, Jenkins publishes the artifact to Nexus, which is an artifact repository manager.
Nexus acts as a centralized storage system for all build artifacts.
By versioning each artifact, Nexus makes it easy to track, retrieve, and deploy specific versions in the future.
This step ensures that all application builds are properly stored and can be accessed when needed for future deployments or rollbacks.

Containerization (Docker):
In our CI/CD pipeline, after building the application and passing code quality checks, we package the application into a Docker image, which contains everything needed to run the application consistently across different environments. This Docker image is then pushed to a Docker registry (like DockerHub or a private registry), making it available for deployment.
